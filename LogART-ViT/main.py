...
def get_args_parser():
    ...
    parser.add_argument("--model", default="deit_tiny",
                        ...
                        help="model")
    parser.add_argument('--config', type=str, default="./configs/3bit/best.py",
    ...
    parser.add_argument("--train-batch-size", default=32,
                        type=int, help="batchsize of training set")
    ...
    optimize_mode_group.add_argument('--optimize', action='store_true', 
    ...
    parser.add_argument('--test_with_hardware_approx', action='store_true', 
                        help='test the model with hardware approximation')
    ...
    parser.add_argument("--iters_w", default=2000, type=int, help="number of iterations in AdaRound")
    parser.add_argument('--rrweight', type=float, default=1, 
                        help='weight of rounding cost vs the reconstruction loss.')
    parser.add_argument('--lr', type=float, default=0.05, help='learning rate in AdaRound')
    parser.add_argument("--scale-method", type=str, default='log_dynamic', 
    ...
    parser.add_argument('--search_method', default='block_wise', type=str, 
                        help='Search method for hyperparameters',
                        choices=['tensor_wise', 'block_wise'])
...
def get_train_samples(train_loader, num_samples=32):
    train_data = []
...
def main(args):
    ...
    cfg.search_method = args.search_method if hasattr(args, 'search_method') else cfg.search_method
    ...
    cfg.w_sym = args.w_sym if hasattr(args, 'w_sym') else cfg.w_sym
    ...
    g = mydatasets.ViTImageNetLoaderGenerator(data_path, args.train_batch_size, args.val_batch_size, args.num_workers, kwargs={"model":model})
    ...
    search_loader = get_train_samples(train_loader, num_samples=cfg.calib_size)
    hyperparameter_search(search_model, model, search_loader, cfg, device=device)
    end_time = time.time()
    ...
    if args.optimize:
        ...
        calib_loader = g.calib_loader(num=cfg.optim_size, batch_size=cfg.optim_batch_size)
        ...
        block_reconstructor = BlockReconstructor(model, full_model, calib_loader, metric=cfg.optim_metric, 
                                                 ...
                                                 iters_w=args.iters_w, rrweight=args.rrweight, lr=args.lr)
    ...
    if args.test_with_hardware_approx:
        for name, module in model.named_modules():
            if hasattr(module, 'w_quantizer'):
                module.w_quantizer.hardware_approx = True    
    ...
    experiment_set = {
        ...
        'if_optimize': args.optimize,
        'search_samples': cfg.calib_size,
